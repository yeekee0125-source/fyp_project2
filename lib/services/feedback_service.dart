...
class FeedbackService {
  ...
  // 1. User: Submit new feedback
  Future<void> submitFeedback({required int rating, required String reason, required String description}) async {
    final initialChat = [{
      'sender': 'user',
      'message': description,
  ...
  // 2. Both: Send a message (Optimized for JSONB)
  Future<void> sendMessage(String feedbackId, String message, String sender) async {
    // Fetch current history first
    final res = await _supabase.from('feedback').select('chat_history').eq('id', feedbackId).single();
    List<dynamic> history = List.from(res['chat_history'] ?? []);
    ...
    await _supabase.from('feedback').update({
      ...
      // If user sends, it goes back to Pending. If admin sends, it's In Progress.
      'status': sender == 'user' ? 'Pending' : 'In Progress',
      'user_replied_at': sender == 'user' ? DateTime.now().toIso8601String() : null,
    }).eq('id', feedbackId);
  }
  ...
  Stream<List<Map<String, dynamic>>> getAdminStream(String status) {
    return _supabase
        .from('feedback')
        .stream(primaryKey: ['id'])
        .order('created_at', ascending: false)
